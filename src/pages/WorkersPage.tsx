...
const WorkersPage = () => {
  ...
  const loadWorkers = async () => {
    ...
    setLoading(true);
    try {
      const snap = await getDocs(collection(db, "contractors", user.uid, "workers"));
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingId) {
        await updateDoc(doc(db, "contractors", user.uid, "workers", editingId), data);
      } else {
        await addDoc(collection(db, "contractors", user.uid, "workers"), {
          ...data,
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteDoc(doc(db, "contractors", user.uid, "workers", showDelete));
      setShowDelete(null);
      await loadWorkers();
    } catch (err) {
  ...
  const workTypeLabels: Record<string, string> = {
    labour: "Labour", helper: "Helper", mistry: "Mistry",
  };
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 text-[10px] text-muted-foreground">
                    <span>{workTypeLabels[w.workType] || w.workType}</span>
                    <span>•</span>
      ...
      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent className="max-w-sm mx-auto max-h-[85vh] overflow-y-auto">
          ...
          <div className="space-y-3 mt-2">
            ...
            <div>
              <label className="text-xs text-muted-foreground font-medium mb-1 block">{t("workType")}</label>
